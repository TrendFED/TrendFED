 ...
 *
 * Date: 2009-06-08 Modify from Potato
 * Icnulude .AJAX to doXHR() function for CM.
 */
	...
	function tree_component () {
		...
		return {
			...
			init : function(elem, opts) {
				...
				// THEMES
				if(this.settings.ui.theme_path === false) this.settings.ui.theme_path = this.path + "tree_themes/";
			...
			context_menu : function () {
				...
				if(this.settings.ui.context != false) {
					...
					for(var i in this.settings.ui.context) {
						...
						//////////////original tree_component script////////////////
						////////////////////////////////////////////////////////////
						/*var icn = ""
						...
						/////////////////////////////////////////////////////////////
						//////////////original tree_component script END/////////////
						
						//////////////modified script///////////////////////////////
						////////////////////////////////////////////////////////////
						var get_class = $(this).attr("class");
						var icn = $(this).val(get_class);
						
						if(this.settings.ui.context[i].icon) icn = this.settings.ui.context[i].icon;
						str += '<a rel="' + this.settings.ui.context[i].id + '" href="#" class="'+icn+'" >' + this.settings.ui.context[i].label + '</a>';
						//////////////modified script END///////////////////////////////
						////////////////////////////////////////////////////////////
					}
					str += '</div>';
			...
			refresh : function (obj) {
				...
				if(this.settings.data.type == "xml_flat" || this.settings.data.type == "xml_nested") {
    				...
    				var error_callback = function(xhr) {
    					$(".msgbox > h1").text(ERROR_CAPTION);
    					$(".msgbox > p").text(SERVER_ERROR_MSG);
    					$(".msgbox").addClass("msgerr");
    					$(".msgbox").show();
    					$('#button-gen').removeAttr("disabled");
    					$('#button-cancel').removeAttr("disabled");
    					return false
    				} //End of Error handling
						doXHR(this.settings.data.url, this.settings.data.async_data(false), callback, error_callback);
					}
				}
				else {
			...
			open_branch : function (obj, disable_animation, callback) {
				...
				if(this.settings.data.async && obj.find("li").size() == 0) {
  				...
  				doXHR(this.settings.data.url, this.settings.data.async_data(obj), callback, error_callback);
			...
			search : function(str) {
				...
				if(this.settings.data.async) {
    				...
    				doXHR(this.settings.data.url, dd, callback, error_callback);
